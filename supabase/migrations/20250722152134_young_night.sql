/*
  ...
  1. Nouvelles Tables
    - `time_off_requests`
      ...
      - `type` (enum: paid, sick, unpaid, personal)
      - `start_date` (date)
      - `end_date` (date)
      - `days` (integer)
      ...
      - `approved_by` (uuid, référence vers employees, optionnel)
      - `approved_date` (timestamptz, optionnel)
  ...
  2. Sécurité
    ...
    - Politique pour que les admins/RH puissent voir toutes les demandes
    - Politique pour que les employés puissent créer leurs propres demandes
    - Politique pour que les admins/RH puissent approuver/rejeter les demandes
*/

-- Créer le type enum pour les types de congés
